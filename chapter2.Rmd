...
- Write `?state` or `help(dataset)` to the console to open up the help page on `state`. Take a look at the variable descriptions under `state.x77`. This is where all but one variable of `US` is described.
...
*** =solution
```{r}
# Look at the counts of different levels of Pop_Area in data frame US.
table(US$PoptoArea)
...
# When executed together, these functions draw a barplot and add x-axis label
barplot(table(US$PoptoArea))
title(xlab="population/area")
...
# Create a summary of Life_Exp in the data frame US
...
# # Draw a histogram of Life_Exp in data frame US. Make it a colored one!
...
# Draw a boxplot of Income in the data frame US. Add a y axis label and a main title.
