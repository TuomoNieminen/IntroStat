...
During this course you will operate on ready-made functions, but for clarity we will look at some of the details on how functions are built.
...
# Here we call the add_5 function twice. ( add_5() is already and saved in memory )
add_5(n = 5)
add_5(n = -5)
...
# Here we call the do_maths function twice. k equals 5 by default so these two calls are equal
do_maths(n = 10, k = 5)
do_maths(n = 10)
...
A vector is an object containing multiple elements. Vectors are quite important in R. The columns a data frame (such as `iris`) are vectors.
...
# This time we'll save it and then compute it's range
...
# Get the 25th, 50th and 75th percentile of Sepal.Width in iris, using quantile(). Use the vector you created above as one of the arguments of quantile().
?quantile
...
One example is the `summary()` function.
...
success_msg("Great! Next we'll take a look at what a class is.")
...
Inspect the structure of `iris` using `str()`. What is the class of `Species` in `iris`
...
# See the structure of a simple numerical object
str(5)


# Take a look at the structure of iris here
...
msg2 = "Numeric is a class in R, but is it not correct here, even though you can see from str() that Species has numbers as labels."
