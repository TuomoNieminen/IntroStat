...
*** =instructions
- Execute the functions `head()` and `mean()`
...
# You can use the $-sign to access columns. Here we compute the mean of sepal lengths.
...
test_function("summary", args="object", 
              incorrect_msg="Did you make a summary of the Petal.Width column in iris, using $ to access it?")
...
During this course you will operate on ready-made functions, but for clarity we will look at some of the details of how functions are built.

A function is called by typing it's name and giving it the necessary *arguments* inside parenthesis.


*** =instructions
- Try the functions that are defined.
- Call `do_maths()` functio with `n` equal to 20 and `k` equal to 10.
...
- Define a vector object that includes the quantile probabilities corresponding to the 25th, 50th, and 75th percentiles.
- Compute the 25th, 50th and 75th percentiles of Sepal.Length using `quantile()`.

*** =hint
-  you need two give 2 arguments to `quantile()`: `x` and `probs`.
- Use `c()` to define `probs`.
...
# Get the 25th, 50th and 75th percentile of Sepal.Width using quantile()
?quantile
...
test_function("quantile", args=c("x","probs"),
              incorrect_answer = "Did you call `quantile()` with two arguments: `x` and `probs` ? ")
